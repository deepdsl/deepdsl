...
public class LmdbUtils {
    ...
    public static byte[] getImageAsByteArray(Transaction tx, Database db, int numSamples, boolean keepTx, OS os) {
        ...
        for (Entry next : iterable) {
            try {
                byte[] src;
                if (os == OS.OSX) {
                    deepdsl.data.imagenet.osx.ImagenetProto.Datum datum = deepdsl.data.imagenet.osx.ImagenetProto.Datum.parseFrom(next.getValue());
                    System.out.println("label = " + datum.getLabel());
                    src = datum.getData().toByteArray();
                } else if (os == OS.LINUX) {
                    deepdsl.data.imagenet.linux.ImagenetProto.Datum datum = deepdsl.data.imagenet.linux.ImagenetProto.Datum.parseFrom(next.getValue());
                    src = datum.getData().toByteArray();
                } else if (os == OS.WINDOWS) {
                    deepdsl.data.imagenet.win32.ImagenetProto.Datum datum = deepdsl.data.imagenet.win32.ImagenetProto.Datum.parseFrom(next.getValue());
                    src = datum.getData().toByteArray();
    ...
    public static Tuple nextTuple(Entry entry, OS os) {
        ...
        try {
            if (os == OS.OSX) {
                deepdsl.data.imagenet.osx.ImagenetProto.Datum datum = deepdsl.data.imagenet.osx.ImagenetProto.Datum.parseFrom(entry.getValue());
                image = datum.getData().toByteArray();
                label = datum.getLabel();
                dims[0] = datum.getChannels(); dims[1] = datum.getHeight(); dims[2] = datum.getWidth();
            } else if (os == OS.LINUX) {
                deepdsl.data.imagenet.linux.ImagenetProto.Datum datum = deepdsl.data.imagenet.linux.ImagenetProto.Datum.parseFrom(entry.getValue());
                image = datum.getData().toByteArray();
                label = datum.getLabel();
                dims[0] = datum.getChannels(); dims[1] = datum.getHeight(); dims[2] = datum.getWidth();
            } else if (os == OS.WINDOWS) {
                deepdsl.data.imagenet.win32.ImagenetProto.Datum datum = deepdsl.data.imagenet.win32.ImagenetProto.Datum.parseFrom(entry.getValue());
                image = datum.getData().toByteArray();
                label = datum.getLabel();
                dims[0] = datum.getChannels(); dims[1] = datum.getHeight(); dims[2] = datum.getWidth();
            } else throw new RuntimeException("The operation system is not supported");
    ...
    public static int[] getDims(Transaction tx, Database db, OS os) {
        try(BufferCursor cursor = db.bufferCursor(tx)) {
            if (cursor.first()) {
                try {
                    if (os == OS.OSX) {
                        deepdsl.data.imagenet.osx.ImagenetProto.Datum datum = deepdsl.data.imagenet.osx.ImagenetProto.Datum.parseFrom(cursor.valBytes());
                        return new int[]{datum.getChannels(), datum.getHeight(), datum.getWidth()};
                    } else if (os == OS.LINUX) {
                        deepdsl.data.imagenet.linux.ImagenetProto.Datum datum = deepdsl.data.imagenet.linux.ImagenetProto.Datum.parseFrom(cursor.valBytes());
                        return new int[]{datum.getChannels(), datum.getHeight(), datum.getWidth()};
                    } else if (os == OS.WINDOWS) {
                        deepdsl.data.imagenet.win32.ImagenetProto.Datum datum = deepdsl.data.imagenet.win32.ImagenetProto.Datum.parseFrom(cursor.valBytes());
                        return new int[]{datum.getChannels(), datum.getHeight(), datum.getWidth()};
                    } else throw new RuntimeException("The operation system is not supported");
                } catch (InvalidProtocolBufferException e) {
                    throw new RuntimeException(e);
