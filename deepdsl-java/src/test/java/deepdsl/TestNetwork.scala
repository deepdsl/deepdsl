...
class TestNetwork {
  ...
  def testResidualNN_cuda {   
    ...
    val solver = Train("rcnn", 100, 10, 0.01f, 0.9f, 0.0005f, 0)
  ...
  def testVgg_cuda {   
   ...
   println(typeof(network))
    ...
    val param = c.freeVar.toList.sortWith((a,b) => a.toString < b.toString)
    val solver = Train("vgg", 2000, 40, 0.01f, 0.9f, 0.0005f, 0)
    val loop = Loop(c, p, (x, y), param, solver)
  ...
  def testOverfeat_cuda {   
   ...
   println(typeof(network))
    ...
    val solver = Train("overfeat", 400, 40, 0.01f, 0.9f, 0.0005f, 0)
    val loop = Loop(c, p, (x, y), param, solver)
  ...
  def testGooglenet_cuda {
    ...
    val solver = Train("googlenet", 1000, 20, 0.01f, 0.9f, 0.0005f, 0)
    val loop = Loop(c, p, (x, y), param, solver)
    runtimeMemory(loop.train)
    parameterMemory(loop)
    workspaceMemory(loop.train)
